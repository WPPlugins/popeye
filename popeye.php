...
define('PPY_JQ_VERSION', '2.0'); //the version of popeye itself is the main version nr of this plugin
define('PPY_WP_PLUGIN_VERSION', '0.2'); //subversion of this wp plugin
...
function ppy_say_hello() 
{
  ?><div id='ppy-hello' class='updated fade'>
    ...
    You can configure its behaviour using the <a href="options-general.php?page=popeye">popeye settings page</a>.
    </p></div><?php 
}
...
function ppy_shortcode_handler($atts, $content=null, $code="")
{
  ...
  //if 'ids' attribute is not set we grab the attached images
	if(!isset($atts['ids'])) 
    $ids = ppy_get_image_attachments_of_post($post->ID);
  ...
  //return html code
  return ppy_generate_html($ids, $options, $useCustomOptions);
}
 ...
 * @return array
 */
function ppy_get_image_attachments_of_post( $postId )
{
  $ids = array();
 ...
 */
function ppy_generate_html(array $ids, array $options, $useCustomOptions = false)
{
  global $ppy_counter;
  $style =& $options['style'];
  $htmlId = 'ppy' . $ppy_counter;
  $classId = $useCustomOptions ? '' : 'ppy'; 
...
function ppy_content_filter($text)
{
  ...
  if($result) 
  {
	...
	//Getting replacement
	$ids = ppy_get_image_attachments_of_post(get_the_ID());
...
{
	add_options_page('Popeye default settings', 'Popeye', 'manage_options', 'popeye', 'ppy_options_page');
}
...
{
  ?><p>Popeye can be inserted into the text by a shortcode [popeye] or automatically at the 
  beginning or randomly within a page or post. Visit the plugin homepage for 
  <a href="http://chrp.es/wp-popeye">usage help</a> and
  <a href="http://chrp.es/wp-popeye-styles">further styles</a></p><?php
}
...
function ppy_options_view_size_output()
{
  ...
  ?><select id="ppy_caption" name="ppy_options[view_size]" size="1" style="width:130px">
...
function ppy_options_enlarge_size_output()
{
  ...
  ?><select id="ppy_caption" name="ppy_options[enlarge_size]" size="1" style="width:130px">
    <option value="medium" <?php   if($opt=='medium')    echo 'selected="selected"' ?>>Medium</option>
    <option value="large" <?php    if($opt=='large')     echo 'selected="selected"' ?>>Large</option>
    <option value="original" <?php if($opt=='full')      echo 'selected="selected"' ?>>Original</option>
  </select><?php
}
...
function ppy_options_validate($input) 
{
  ...
  foreach($output as $option => &$outVal)
  {
    $inVal = isset($input[$option]) ? strtolower($input[$option]) : null;
  	switch($option)
    {
      case 'duration':
      	if($inVal >= 0 && $inVal <= 5000) $outVal = $inVal; 
        break;
        
      case 'alignment':
      	if(in_array($inVal, array('left','right'))) $outVal = $inVal;
        break;
        
      case 'opacity':
        if($inVal > 1) $inVal = $inVal / 100;
        if($inVal >= 0 && $inVal <= 1) $outVal = $inVal;
        else if($inVal < 0) $outVal = 0;
        else $outVal = 1;
      	break;
        
      case 'view_size':
      	if(in_array($inVal, array('thumbnail', 'medium','large'))) $outVal = $inVal;
        break;

      case 'enlarge_size':
        if(in_array($inVal, array('medium','large','full'))) $outVal = $inVal;
        break;
      ...
      case 'mode':
      	if(in_array($inVal, array('prepend','random', 'manual', 'second', 'third'))) $outVal = $inVal;
        break;
    }
  }
...
function ppy_get_options($style = null, $ignoreStyleOptions = false)
{
	...
	return !$ignoreStyleOptions ?
		ppy_override_by_style_options(
		  empty($style) ? ppy_get_current_style() : $style,
		  $options
		)
		: $options;
}
...
function ppy_override_by_style_options($style, $options = array())
{
	...
	$rewritableOptions = array('caption', 'navigation', 'opacity', 'duration', 'z-index', 'easing', 'debug');
